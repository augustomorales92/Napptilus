...
const ProductsDetailsPage = () => {
  ...
  const {
    ...
    isLoading,
    status
  } = useQuery(['phones', params?.id], () => getDataById(params?.id))
    ...
    if (status === 'success') {
      ...
      const colorCode = String(
        Array.isArray(options.colors)
          ? options.colors[0].code
          : options.colors.code
      )
      const storageCode = String(
        Array.isArray(options.storages)
          ? options.storages[0].code
          : options.storages.code
      )
      ...
      <div className="row row-cols-1 table-responsive m-4 table-responsive-{sm | md | lg | xl | xxl}">
        <div className="card text-bg-dark cardMobile">
          <div className="d-flex justify-content-center  align-items-center col">
            <img
              src={product?.imgUrl}
              className="img-thumbnail white-shadow imageSize"
              alt="img"
            />
          </div>
          <div className="col mx-auto ">
            ...
            <div className="d-flex justify-content-around align-items-center">
              <button
                className=" d-flex justify-content-around btn btn-outline-light mt-4 mw-100 mb-4 rem-12 back-home "
                onClick={() => navigate('/')}
              >
              ...
              <button
                type="button"
                className="d-flex justify-content-around btn btn-outline-success mt-4 mw-100 mb-4 rem-12 "
                onClick={addToCart}
              >
                {' '}
